...
\item le rayon réfléchi et le rayon réfracté (lorsqu'il existe) sont contenus dans le \tdef{plan d'incidence}, qui est le plan contenant le rayon incident et la normale $(\mathcal{N})$ au dioptre au point d'incidence $\point{I}$. On a donc le schéma suivant :
...
\draw [dashed] (I) +(0,-0.8) -- +(0,1.5) node [above] {$(\mathcal{N})$};
...
\draw [->] (2.5,0) arc (0:90:0.5) node [midway, below left] {$+$}; 
\end{tikzpicture}
\end{center}
\end{figure}
